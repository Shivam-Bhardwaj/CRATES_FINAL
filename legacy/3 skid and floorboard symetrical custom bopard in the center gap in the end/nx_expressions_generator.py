...
def generate_crate_expressions_logic(
    ...
    try:
        ...
        # === SKID CALCULATIONS (Re-verified for symmetry) ===
        ...
        crate_overall_width_od_in = product_width_in + (2 * clearance_each_side_in) # This IS the floorboard length
        ...
        # === FLOORBOARD CALCULATIONS (REFINED SYMMETRIC FILL v7) ===
        fb_actual_length_in = crate_overall_width_od_in # Floorboard length IS the crate's overall width
        ...
        # Step 1: Fill from outsides with standard boards
        boards_group1_widths = [] # Boards from start towards center
        boards_group2_widths = [] # Boards from end towards center (will be reversed later)
        y_covered_by_std_boards = 0.0
        remaining_y_for_std_fill = fb_usable_coverage_y_in

        while True:
            placed_this_iteration = False
            # Try placing at the "front" (start of boards_group1_widths)
            for std_w in sorted_std_lumber_widths_available:
                if std_w <= remaining_y_for_std_fill + 0.001:
                    boards_group1_widths.append(std_w)
                    y_covered_by_std_boards += std_w
                    remaining_y_for_std_fill -= std_w
                    placed_this_iteration = True
                    break 
            
            if not placed_this_iteration: break # Cannot fit any more standard boards from the front

            # Try placing a matching or largest possible at the "back"
            # This is a simplification; true symmetric fill is more complex.
            # For now, we are greedily filling with standard boards.
            # The "symmetric" aspect will come from how the custom/gap is centered.
            # The following block for boards_group2_widths is removed to simplify to a greedy fill first.
            # The old symmetric fill for standard boards was too complex and potentially causing issues.
            
        # All standard boards that can fit are now in boards_group1_widths
        # current_y_covered_by_lumber is y_covered_by_std_boards
        # remaining_y_for_std_fill is the space left for custom/gap

        # Step 2: Address the remaining central space
        center_custom_board_width = 0.0
        actual_middle_gap = 0.0
        
        if remaining_y_for_std_fill >= min_custom_lumber_width_in - 0.001:
            center_custom_board_width = remaining_y_for_std_fill
            # current_y_covered_by_lumber += center_custom_board_width # Already accounted for as it fills the remainder
        elif 0.001 < remaining_y_for_std_fill <= max_allowable_middle_gap_in + 0.001:
            actual_middle_gap = remaining_y_for_std_fill
        # else: remaining space is too small for gap and too small for custom - becomes unused space

        # Step 3: Arrange the boards for layout
        all_layout_pieces = [] # Will store widths of boards and the gap value
        
        # For simplicity, assume standard boards are laid out, then custom/gap, then remaining std.
        # With the greedy fill above, boards_group1_widths contains all standard boards.
        
        # The layout will be: [all standard boards from boards_group1_widths] 
        #                      followed by [EITHER custom_board OR middle_gap if one of them exists]

        all_layout_pieces.extend(boards_group1_widths)
        
        # The center_custom_board_width and actual_middle_gap are mutually exclusive 
        # based on the logic in Step 2. One of them will be > 0, or both are 0.

        floorboards_data = []
        current_y_pos = fb_initial_start_y_offset_abs
        
        # To truly center a custom board or gap, we need to lay out roughly half the standard boards,
        # then the custom/gap, then the other half.

        num_std_boards_for_layout = len(boards_group1_widths)
        split_idx = math.ceil(num_std_boards_for_layout / 2.0)

        first_half_std = boards_group1_widths[:int(split_idx)]
        second_half_std = boards_group1_widths[int(split_idx):]

        for board_w in first_half_std:
            floorboards_data.append({'width': board_w, 'y_pos': current_y_pos})
            current_y_pos += board_w
        
        if center_custom_board_width > 0.001:
            floorboards_data.append({'width': center_custom_board_width, 'y_pos': current_y_pos})
            current_y_pos += center_custom_board_width
        elif actual_middle_gap > 0.001:
            current_y_pos += actual_middle_gap # Advance position by gap width

        for board_w in second_half_std:
            floorboards_data.append({'width': board_w, 'y_pos': current_y_pos})
            current_y_pos += board_w
            
        total_width_of_placed_lumber_final = sum(b['width'] for b in floorboards_data)
        gap_if_no_custom = actual_middle_gap if not (center_custom_board_width > 0.001) else 0.0
        final_floor_gap_debug_val = fb_usable_coverage_y_in - (total_width_of_placed_lumber_final + gap_if_no_custom)

        # --- Prepare expressions file content ---
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        expressions_content = [
            f"// NX Expressions - Skids & Floorboards (Refined Symmetric Fill v7)",
...
class CrateApp: 
    def __init__(self, master):
        self.master = master; master.title("NX Crate Exporter (Refined Symmetric Fill v7)") 
    ...
    def generate_file(self): 
        ...
        output_filename = filedialog.asksaveasfilename(defaultextension=".exp", initialfile="Crate_RefinedSymmetricFill_v7.exp") # Updated filename
