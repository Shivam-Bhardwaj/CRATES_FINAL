...
def generate_crate_expressions_logic(
    ...
    try:
        ...
        expressions_content.append(f"\n// --- PANEL BOUNDING BOX PARAMETERS ---")
        expressions_content.append(f"[Inch]PANEL_Front_Calc_Width = {front_panel_calc_width:.3f}")
        expressions_content.append(f"[Inch]PANEL_Front_Calc_Height = {front_panel_calc_height:.3f}")
        expressions_content.append(f"[Inch]PANEL_Front_Calc_Depth = {front_panel_calc_depth:.3f}\n")
        expressions_content.append(f"[Inch]PANEL_Back_Calc_Width = {back_panel_calc_width:.3f}")
        expressions_content.append(f"[Inch]PANEL_Back_Calc_Height = {back_panel_calc_height:.3f}")
        expressions_content.append(f"[Inch]PANEL_Back_Calc_Depth = {back_panel_calc_depth:.3f}\n")
        expressions_content.append(f"[Inch]PANEL_End_Calc_Length = {end_panel_calc_length:.3f} // For Left & Right End Panels")
        expressions_content.append(f"[Inch]PANEL_End_Calc_Height = {end_panel_calc_height:.3f} // For Left & Right End Panels")
        expressions_content.append(f"[Inch]PANEL_End_Calc_Depth = {end_panel_calc_depth:.3f} // For Left & Right End Panels\n")
        expressions_content.append(f"[Inch]PANEL_Top_Calc_Width = {top_panel_calc_width:.3f}")
        expressions_content.append(f"[Inch]PANEL_Top_Calc_Length = {top_panel_calc_length:.3f}")
        expressions_content.append(f"[Inch]PANEL_Top_Calc_Depth = {top_panel_calc_depth:.3f}\n")
...
class CrateApp: 
    def __init__(self, master):
        ...
        ttk.Label(general_frame, text="Product Actual Height (in):").grid(row=2, column=0, sticky="w", pady=2); self.product_actual_height_entry = ttk.Entry(general_frame, width=25); self.product_actual_height_entry.grid(row=2, column=1, sticky="ew", pady=2); self.product_actual_height_entry.insert(0,"50.0")
        ttk.Label(general_frame, text="Clearance Above Product (in):").grid(row=3, column=0, sticky="w", pady=2); self.clearance_above_product_entry = ttk.Entry(general_frame, width=25); self.clearance_above_product_entry.grid(row=3, column=1, sticky="ew", pady=2); self.clearance_above_product_entry.insert(0,"2.0")
        ttk.Label(general_frame, text="Ground Clearance (End Panels, in):").grid(row=4, column=0, sticky="w", pady=2); self.ground_clearance_entry = ttk.Entry(general_frame, width=25); self.ground_clearance_entry.grid(row=4, column=1, sticky="ew", pady=2); self.ground_clearance_entry.insert(0,"1.0")
