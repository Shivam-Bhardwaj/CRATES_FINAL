...
class CrateGeneratorUI(QMainWindow):
    def __init__(self, generation_callback):
        ...
        constr_fields = [
            ...
            ("Ground Clearance (in):", "1.0", "ground_clear", "Ground clearance at ends"), # This is for end panels, distinct from the general one.
            ("Max Middle Gap (in):", "0.25", "max_gap", "Maximum allowable floor gap"),
            ("Min Custom Width (in):", "2.5", "min_custom", "Minimum custom board width"),
            ("Skid Material Nom. Width (in):", "3.5", "skid_mat_nom_w", "Nominal width of skid lumber (e.g., 3.5 for 4xN)"),
            ("Skid Material Nom. Thick. (in):", "1.5", "skid_mat_nom_t", "Nominal thickness of skid lumber (e.g., 1.5 for Nx2)"),
            ("Skid Spacing (in):", "24.0", "skid_spacing", "Center-to-center spacing between skids"),
            ("Max Inter-Cleat Spacing (in):", "24.0", "max_inter_cleat_spacing", "Maximum spacing between panel cleats"),
            ("Num Skids Override (opt.):", "", "num_skids_override", "Optional: specify exact number of skids")
        ]
        ...
        # Adjust row indices for checkboxes due to more fields
        constr_grid.addWidget(self.allow_3x4, 4, 0, 1, 6)
        constr_grid.addWidget(self.force_small_custom, 5, 0, 1, 6)
    ...
    def get_user_inputs(self):
        try:
            ...
            ground_clearance = float(self.ground_clearance.text()) # This is the general ground_clearance_in
            ...
            # New fields from constr_fields
            skid_mat_nom_w = float(self.skid_mat_nom_w.text())
            skid_mat_nom_t = float(self.skid_mat_nom_t.text())
            skid_spacing = float(self.skid_spacing.text())
            max_inter_cleat_spacing = float(self.max_inter_cleat_spacing.text())
            num_skids_override_str = self.num_skids_override.text()
            num_skids_override = int(num_skids_override_str) if num_skids_override_str else None
            ...
            inputs = {
                ...
                'ground_clearance_in': ground_clearance, # From general_group
                ...
                # Adding new keys for orchestrator
                'skid_material_nominal_width_in': skid_mat_nom_w,
                'skid_material_nominal_thickness_in': skid_mat_nom_t,
                'skid_spacing_in': skid_spacing,
                'max_inter_cleat_spacing_in': max_inter_cleat_spacing,
                'num_skids_override': num_skids_override
            }
            return True, inputs
