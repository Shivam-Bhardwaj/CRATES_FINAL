...
MIN_FORCEABLE_CUSTOM_BOARD_WIDTH = 0.25 # New constant
...
def generate_crate_expressions_logic(
    ...
    force_small_custom_board_bool: bool, # New parameter
    output_filename: str
) -> tuple[bool, str]:
    try:
        # --- Input Validations ---
        ...
        if min_custom_lumber_width_in < MIN_FORCEABLE_CUSTOM_BOARD_WIDTH:
            return False, f"Min custom board width cannot be less than {MIN_FORCEABLE_CUSTOM_BOARD_WIDTH} inches."


        # === SKID CALCULATIONS (UNCHANGED) ===
        ...
        # === FLOORBOARD CALCULATIONS (Force Small Custom Option) ===
        ...
        # Handle remainder: custom board or gap, with override for small custom board
        if force_small_custom_board_bool and \
           MIN_FORCEABLE_CUSTOM_BOARD_WIDTH - 0.001 <= y_remaining_for_lumber < min_custom_lumber_width_in + 0.001 :
            ...
            actual_middle_gap = 0.0 # Forced small custom board takes precedence
        elif y_remaining_for_lumber >= min_custom_lumber_width_in - 0.001:
            center_custom_board_width = y_remaining_for_lumber
            all_lumber_pieces.append(center_custom_board_width)
            y_covered_by_lumber += center_custom_board_width
            actual_middle_gap = 0.0 # Custom board takes precedence
        elif 0.001 < y_remaining_for_lumber <= max_allowable_middle_gap_in + 0.001:
            actual_middle_gap = y_remaining_for_lumber
            # y_covered_by_lumber remains as is, gap will fill this space
        # Else: remaining space is too small for a gap or custom, becomes unused space (actual_middle_gap remains 0)
        
        # Final check on actual_middle_gap if a custom board was placed (it should be 0)
        if center_custom_board_width > 0.001:
            actual_middle_gap = fb_usable_coverage_y_in - y_covered_by_lumber # Recalculate, should be ~0
            if not (0.001 < actual_middle_gap <= max_allowable_middle_gap_in + 0.001): # if it's not a valid small gap...
                 actual_middle_gap = 0.0 # then there is no gap.

        # Layout the boards with the gap/custom board in the middle of the LUMBER sequence
        floorboards_data = []
        ...
        for i, board_w_val in enumerate(all_lumber_pieces):
            # The custom board is already part of all_lumber_pieces if it was determined
            floorboards_data.append({'width': board_w_val, 'y_pos': current_y_pos})
            current_y_pos += board_w_val
            
            if i == gap_insertion_index and actual_middle_gap > 0.001 and center_custom_board_width <= 0.001: # Only insert gap if no custom board is filling center
                current_y_pos += actual_middle_gap
        
        total_lumber_width_in_data = sum(b['width'] for b in floorboards_data)
        effective_gap_in_data = 0.0
        if actual_middle_gap > 0.001 and center_custom_board_width <= 0.001: # Check if gap was intended and no custom board
            if gap_insertion_index != -1 and gap_insertion_index < num_lumber_pieces_for_layout -1 : # and it wasn't after the last board
                 effective_gap_in_data = actual_middle_gap
            elif num_lumber_pieces_for_layout == 1 and gap_insertion_index == 0: # single board, gap after it
                 effective_gap_in_data = actual_middle_gap


        final_floor_gap_debug_val = fb_usable_coverage_y_in - (total_lumber_width_in_data + effective_gap_in_data)

        # --- Prepare expressions file content ---
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        expressions_content = [
            f"// NX Expressions - Skids & Floorboards (Force Small Custom Option v1)",
...
class CrateApp: 
    def __init__(self, master):
        self.master = master; master.title("NX Crate Exporter (Force Small Custom)") 
        master.geometry("550x780") # Slightly taller for new checkbox
        ...
        # **NEW: Force Small Custom Board Checkbox**
        self.force_small_custom_var = tk.BooleanVar(value=False) # Default to not forcing
        self.force_small_custom_cb = ttk.Checkbutton(floor_frame, 
            text=f"Force small custom board to fill tiny gap ({MIN_FORCEABLE_CUSTOM_BOARD_WIDTH}\" - {DEFAULT_MIN_CUSTOM_LUMBER_WIDTH}\")", 
        ...
        lumber_select_frame.grid(row=4, column=0, columnspan=2, pady=5, sticky="ew") # Adjusted row
        self.lumber_vars = {}
        max_cols = 2 
        ...
        self.generate_button.grid(row=4, column=0, columnspan=2, pady=20) # Adjusted row
    ...
    def generate_file(self): 
        try:
            ...
            force_small_custom = self.force_small_custom_var.get() # Get new option
        ...
        output_filename = filedialog.asksaveasfilename(defaultextension=".exp", initialfile="Crate_ForceSmallCustom.exp")
        ...
        success, message = generate_crate_expressions_logic(
            ...
            force_small_custom_board_bool=force_small_custom, # Pass new option
